...
if(isset($_POST['guardar'])){
	...
	if($suma1>0){
		...
		$sql="insert into gestion (horas, nombre_proyecto) values ('$suma1', '$nombre1') ";
...
if(!$result){
	...
	if($suma2>0){
		$con=mysqli_connect("localhost","root","","elefont")or die("no se ha conectado a la base de datos");
		$sql="insert into gestion (horas, nombre_proyecto) values ('$suma2', '$nombre2') ";
		$result=mysqli_query($con,$sql);
if(!$result){
	$error=mysqli_error($con);
	echo "$error";
	...
	if($suma3>0){
		$con=mysqli_connect("localhost","root","","elefont")or die("no se ha conectado a la base de datos");
		$sql="insert into gestion (horas, nombre_proyecto) values ('$suma3', '$nombre3') ";
		$result=mysqli_query($con,$sql);
if(!$result){
	$error=mysqli_error($con);
	echo "$error";
	...
	if($suma4>0){
		$con=mysqli_connect("localhost","root","","elefont")or die("no se ha conectado a la base de datos");
		$sql="insert into gestion (horas, nombre_proyecto) values ('$suma4', '$nombre4') ";
		$result=mysqli_query($con,$sql);
if(!$result){
	$error=mysqli_error($con);
	echo "$error";
